...
| Admin login | URL separada `/admin-login` con email + password |
...
- [x] **2.2** `auth.py` — Endpoint `POST /auth/login-admin`: `email + password` solo para administradores
...
- [ ] **3.1** `router/index.ts` — Rutas públicas: `/kds`, `/checkin`, `/admin-login`; `/login` sin guard pero sin admins
- [ ] **3.2** **[NUEVO]** `AdminLogin.vue` — Ruta `/admin-login`, formulario email + password, sin visibilidad hacia staff
- [ ] **3.3** `Login.vue` — Destruir formulario email/password
- [ ] **3.4** `Login.vue` — Primer arranque: selector de rol por dispositivo (guarda en localStorage)
- [ ] **3.5** `Login.vue` — Grid de avatares filtrado por rol del dispositivo (excluye admins siempre)
- [ ] **3.6** `Login.vue` — Overlay keypad numérico al seleccionar usuario → `POST /auth/token`
- [ ] **3.7** `Login.vue` — Botón secundario discreto → `/checkin`
...
| Admin login | Admin entra por `/admin-login` con email+password, no aparece en grid staff |
