  ...
  <div class="min-h-screen bg-[#000a20] text-white overflow-x-hidden font-sans pb-10">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3 md:py-4">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-6">
          <div class="flex items-center gap-4">
            <div class="p-3 md:p-4 bg-gradient-to-br from-[#FDB700] to-[#FFD700] rounded-xl md:rounded-2xl shadow-lg">
              <LayoutDashboard class="w-7 h-7 text-[#00126D]" />
            </div>
            <div>
              <h1 class="text-2xl font-black tracking-tighter uppercase leading-none">Control de Cocina</h1>
              <div class="flex items-center gap-2 mt-1">
                ...
                <p class="text-slate-400 text-[9px] font-black tracking-[0.2em] uppercase opacity-70">
                  {{ pedidosStore.wsConnected ? 'En línea' : 'Reconectando...' }}
                </p>
              </div>
            </div>
          </div>
          ...
          <!-- Navigation Tabs -->
          <div class="flex w-full md:w-auto bg-black/40 p-1 rounded-2xl border border-white/10 shadow-inner">
            <button
              @click="vistaActual = 'activos'"
              class="flex-1 md:flex-auto flex items-center justify-center gap-2 px-3 md:px-6 py-3 rounded-xl text-xs font-black transition-all duration-300 tracking-wider"
    ...
    <div class="relative z-10 max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div v-if="vistaActual === 'disponibilidad'" class="space-y-8 animate-in active">
        <div class="bg-[#00126D]/10 backdrop-blur-xl rounded-2xl md:rounded-[2rem] p-4 md:p-8 border border-white/5 shadow-2xl">
          <div class="flex flex-col sm:flex-row items-center gap-4 md:gap-8">
            ...
            <div class="flex-1 w-full">
              <h2 class="text-xl md:text-3xl font-black tracking-tight mb-3 uppercase">Gestión de Stock Activo</h2>
              <div class="relative">
                ...
                <input
                  ...
                  class="w-full pl-14 pr-8 py-3 md:py-5 bg-black/40 border border-white/10 rounded-2xl md:rounded-3xl focus:ring-4 focus:ring-violet-500/20 focus:border-violet-500 outline-none transition-all font-black tracking-wide text-base md:text-xl"
                />
              </div>
            </div>
          </div>
        ...
        <div v-else class="space-y-8">
          <div v-for="(platillosCategoria, categoria) in platillosPorCategoria" :key="categoria" 
               class="bg-white/5 backdrop-blur-md rounded-2xl md:rounded-[2rem] p-4 md:p-8 border border-white/5 shadow-xl">
            <div class="flex items-center justify-between mb-6 md:mb-10 border-b border-white/10 pb-4 md:pb-6">
              <h3 class="text-xl md:text-3xl font-black text-white uppercase tracking-tighter">{{ categoria }}</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div v-for="platillo in platillosCategoria" :key="platillo.id"
                   class="group/item p-4 md:p-6 rounded-2xl md:rounded-[1.5rem] border-2 transition-all duration-300 flex flex-col justify-between min-h-[140px] md:min-h-[160px]"
                   :class="platillo.estado === 'disponible' ? 'bg-[#00126D]/20 border-emerald-500/40' : 'bg-rose-900/20 border-rose-500/60'">
                
                <div class="flex justify-between items-start mb-4">
                  <div class="flex-1 pr-6">
                    <h4 class="font-black text-lg md:text-2xl uppercase tracking-tighter leading-[1.1] mb-2 group-hover/item:text-[#FDB700] transition-colors">
                      {{ platillo.nombre }}
                    </h4>
                    <p class="text-slate-500 font-bold text-xs tracking-widest">PRECIO U.: ${{ Number(platillo.precio).toFixed(2) }}</p>
                  </div>
                  
                  <button @click="toggleDisponibilidad(platillo)" 
                          class="relative w-20 h-12 md:h-10 rounded-full transition-all flex-shrink-0 border-2"
      ...
      <div v-else class="animate-in active">
        <div v-if="pedidosMostrados.length === 0" class="flex flex-col items-center justify-center py-20 md:py-40 opacity-40">
          <div class="p-8 md:p-12 bg-white/5 rounded-full mb-6 md:mb-8 border border-white/10 shadow-2xl">
            <ShieldCheck class="w-16 h-16 md:w-24 md:h-24 text-slate-600" />
          </div>
          <h2 class="text-3xl md:text-5xl font-black uppercase tracking-tighter text-white">Cocina Limpia</h2>
          <p class="text-[#FDB700] mt-3 font-bold tracking-[0.4em] text-sm uppercase">Sin pedidos activos</p>
        </div>

        <div v-else class="space-y-6">
          <div v-for="p in pedidosMostrados" :key="p.id"
               class="relative group rounded-2xl md:rounded-[2rem] border-2 p-4 md:p-6 lg:p-8 transition-all duration-500"
               :class="[getEstadoStyles(p.estado)]">
            
            <div class="flex flex-col lg:flex-row lg:items-center gap-3 md:gap-4 lg:gap-10">
              <!-- Ticket Meta -->
              <div class="flex items-center gap-4 w-full lg:w-auto lg:min-w-[260px]">
                <div class="p-3 md:p-5 bg-black/30 rounded-2xl md:rounded-3xl border border-white/10 shadow-inner">
                  <component :is="getTipoOrdenIcon(p.tipo_orden)" class="w-8 h-8 md:w-10 md:h-10 text-white" />
                </div>
                <div>
                  <span class="text-[11px] font-black text-[#FDB700] uppercase tracking-[0.3em] mb-1 block">TICKET #{{ p.numero_display }}</span>
                  <p class="text-2xl md:text-4xl font-black leading-none uppercase tracking-tighter text-white">
                    {{ p.mesa ? 'Mesa ' + p.mesa : (p.nombre_cliente || 'Para Llevar') }}
                  </p>
                </div>
              </div>
              ...
              <div class="flex flex-col sm:flex-row gap-3 md:gap-4 w-full lg:w-auto">
                <template v-if="vistaActual === 'completados'">
                  <button @click="recuperarPedido(p)"
                          class="flex items-center justify-center gap-3 bg-rose-600 hover:bg-rose-500 text-white font-black px-6 py-4 md:px-10 md:py-5 rounded-2xl md:rounded-[2rem] transition-all shadow-2xl active:scale-95 group/btn border border-rose-400/50 w-full lg:w-auto">
                    <RotateCcw class="w-6 h-6 md:w-7 md:h-7 group-hover/btn:rotate-180 transition-transform duration-500" />
                    <span class="text-sm md:text-lg uppercase tracking-widest font-black">Reactivar</span>
                  </button>
                </template>
                <template v-else>
                  ...
                  <div v-else class="flex items-center gap-3 px-6 py-4 md:px-10 md:py-5 bg-black/40 rounded-2xl md:rounded-[2rem] border border-white/10 opacity-70 w-full lg:w-auto">
                    <ShieldCheck class="w-6 h-6 md:w-8 md:h-8 text-white opacity-40" />
                    <span class="text-white font-black uppercase tracking-widest text-base md:text-lg opacity-40">Archivado</span>
                  </div>
                </template>
              </div>
            </div>
            ...
            <transition enter-active-class="animate-in active" leave-active-class="opacity-0 duration-200">
                ...
                <div v-for="item in (p.articulos_pedido || [])" :key="item.id"
                  ...
                  <div class="flex-1 pr-6">
                    <div class="flex items-center gap-4 mb-2">
                      <div class="w-10 h-10 md:w-12 md:h-12 bg-[#FDB700]/10 rounded-2xl flex items-center justify-center border border-[#FDB700]/30 shadow-inner">
                        <span class="text-2xl md:text-3xl font-black text-[#FDB700]">{{ item.cantidad }}</span>
                      </div>
                      <p class="text-base md:text-xl font-black leading-tight uppercase truncate text-white">{{ item.platillo?.kds_name || item.platillo?.nombre }}</p>
                    </div>
              ...
              <div v-else class="mt-4 md:mt-8 flex items-center gap-4">
                <div class="flex -space-x-4">
                   <div v-for="i in Math.min(p.articulos_pedido?.length || 0, 5)" :key="i" class="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-[#00126D] bg-white/5 overflow-hidden flex items-center justify-center">
                     <Utensils class="w-3 h-3 md:w-4 md:h-4 text-white opacity-30" />
                   </div>
                </div>
                <span class="text-sm md:text-base font-black uppercase tracking-[0.3em] text-white/70">{{ p.articulos_pedido?.length || 0 }} artículos por preparar</span>
              </div>
            </transition>
          </div>
        </div>
...
::-webkit-scrollbar-thumb {
  background: rgba(253,183,0,0.25);
  border-radius: 20px;
  border: 3px solid transparent;
  background-clip: padding-box;
}
::-webkit-scrollbar-thumb:hover {
  background: rgba(253,183,0,0.4);
}
