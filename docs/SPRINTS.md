...
| 1.2 | Purgar el secreto de `alembic.ini:63`; la URL sale solo de env vía `env.py` | `alembic.ini`, `alembic/env.py` | security-auditor | 🟢 — literal borrado de `alembic.ini`; se encontró y arregló un bug de path en `env.py` (apuntaba a `apps/backend/backend/.env`, inexistente post-monorepo, por lo que el `.env` local nunca se cargaba); credencial vieja sigue en historial de git (instancia Neon apagada, riesgo bajo — rotar si "solo sin uso") |
| 1.3 | Borrar `pydantic_settings.py` (shim que eclipsa al paquete real) y `add_propinas_columns.py`; crear `backend/.env.example` | `backend/` raíz | task-executor | 🟢 |
| 1.4 | `app/deps.py` con `require_roles` + consolidar `get_turno_activo` (hoy 4 copias) | `backend/app/deps.py` (nuevo) | backend-architect | 🟢 — `deps.py` creado con `require_roles` (cuelga de `get_current_active_user`) y `get_turno_activo`/`obtener_turno_activo`; en realidad eran 5 sitios (2 funciones nombradas + 3 queries inline), todos idénticos, sin divergencias de negocio; usuario inactivo da 400 (no 403) porque así ya lo hace `get_current_active_user` en ~40 endpoints — cambiarlo es fuera de scope; routers aún no migrados a usarlo (eso es 1.5) |
| 1.5A | Migrar checks de rol — `pedidos.py`, `turnos.py` | 2 routers | task-executor | 🟢 — 7 endpoints migrados en `pedidos.py`; `turnos.py` sin cambios (sus 4 checks son filtros de query por rol, no gates de autorización) |
| 1.5B | Migrar checks de rol — `gastos.py`, `asistencia.py`, `admin.py` | 3 routers | task-executor | 🟢 — 13 endpoints migrados; 6 checks residuales en `gastos.py` son lógica de negocio (filtrado de datos para rol `compras`), no autorización |
| 1.5C | Migrar checks de rol — `users.py`, `reportes.py`, `propinas.py`, `products.py` | 4 routers | task-executor | 🟢 — 10 endpoints migrados; `reportes.py` no tenía checks de rol que migrar; 2 residuales en `propinas.py` son lógica de negocio (filtrado por rol `cajero`) |
| 1.6 | `domain/estados.py` + `constants/estados.ts` como fuente única (sin tabla de transiciones todavía) | backend + 4 consumidores front | backend-architect + vue-ui-architect | 🟢 — `EstadoPedido`/`EstadoArticuloPedido`/`EstadoTurno` como `StrEnum` en backend; `constants/estados.ts` espejo en frontend, migrados `types.ts`, `stores/pedidos.ts`, `KDSManager.vue`, `TurnosSection.vue`. Deuda para S2: comentario desactualizado en `models.py:95`, falta `CheckConstraint` en `pedidos.estado` |
| 1.7 | Seguridad crítica: quitar fallback texto plano, rate limiting en logins, gates en `/usuarios/` y `/ws/stats`, `/health/database` sin host, CORS desde env, JWT de sesión fuera del query string de los `/ws/*` (hallazgo nuevo, confirmado en logs de Railway) | `auth.py`, `config.py`, `main.py`, `users.py`, `websocket_routes.py` | security-auditor + task-executor | 🟢 — blockers resueltos (0 PINs en texto plano en prod, verificado con SELECT de solo lectura). Fallback texto plano eliminado de `verify_password`; rate limiting propio (por IP + por cuenta, ventana deslizante 8/300s) en `/auth/login`, `/auth/login-simple`, `/auth/login-admin` **y** `/auth/asistencia` (mismo oráculo de fuerza-bruta, no estaba en la lista original); `GET /usuarios/` gateado a `administrador`; JWT de WS movido del query string a un frame de auth post-connect (`{"type":"auth","token":...}`), frontend actualizado; `/ws/stats` gateado a `administrador`. 42 tests backend + build frontend verdes. Deuda nueva para triage: `ACCESS_TOKEN_EXPIRE_MINUTES=1440` sin refresh token (alto, requiere feature nueva); rate limit por IP puede colapsar a un bucket global detrás del proxy de Railway sin `--forwarded-allow-ips` (medio, operacional); JWT sigue en `localStorage` del frontend, expuesto a XSS aunque ya no a logs (bajo) |
| 1.8 | `docker-compose.yml` (app + Postgres; Redis llega en S2) | raíz | backend-architect | 🟢 |
| 1.9 | Arreglar `conftest.py` para que los tests nuevos corran | `backend/tests/conftest.py` | backend-architect | 🟢 — arreglado bug real: el seed de tests usaba UTC pero los endpoints filtran por día en `MEXICO_TZ`, causando 2 fallas horario-dependientes; fixture `como_rol(rol)` nuevo para tests con rol arbitrario; limpiados imports muertos post-1.5 en `admin.py`/`asistencia.py`. Suite: 15/15 verde, ruff limpio. Nota: env de CI (`DATABASE_URL_TEST` con `+asyncpg`) es peso muerto — la suite corre sobre SQLite in-memory, no Postgres; decisión pendiente de Leo |
